...
export default function TransactionsPage() {
  ...
  const [debugInfo, setDebugInfo] = useState<{ status?: number; length?: number; raw?: any } | null>(null);
  // no longer need categoryMap: backend returns category_name and icon

  // fetchTransactions defined so we can call refresh manually
  ...
  setDebugInfo({ status: res.status, length: Array.isArray(data) ? data.length : 0, raw: payload });

      const mapped: Transaction[] = Array.isArray(data)
        ? data.map((t: any) => {
            // Backend returns `amount` as string and `created_at` as timestamp
            const amountVal = Number(t.amount ?? t.total_amount ?? 0);
            const created = new Date(
              t.created_at ?? t.transaction_date ?? Date.now(),
            );
            // Normalize note: treat empty or placeholder 'string' as absent
            ...
            return {
              ...
              // prefer backend-provided icon & category_name
      ...
      // Always set fetched transactions (if empty, UI will reflect no items).
      setTransactions(mapped);
        } catch (err) {
      console.error("Lỗi khi lấy transactions:", err);
      setError("Không thể lấy lịch sử giao dịch. Hiển thị dữ liệu mẫu.");
      // fallback to mock data so the user still sees something
  ...
  return (
    ...
    <main className="w-full max-w-md mx-auto h-screen flex flex-col bg-[#f9f9fe] overflow-hidden">
      ...
      <div className="flex-grow overflow-y-auto px-6 pb-32 scrollbar-hide">
        {loading && (
          <div className="w-full py-3 text-center bg-yellow-50 text-yellow-800">
            Đang tải lịch sử giao dịch...
          </div>
        )}
        {error && (
          <div className="w-full py-3 text-center bg-red-50 text-red-800">
            {error}
          </div>
        )}
          ...
          {activeTab === "all" ? (
            <>
              {filteredTransactions.length === 0 && !loading ? (
                <div className="bg-white p-6 rounded-2xl text-center text-[#767681]">
                  Không có giao dịch nào.
                  <div className="mt-3">
                    <button
                      onClick={() => fetchTransactions()}
                      className="px-4 py-2 bg-[#4b5b9a] text-white rounded-lg"
                    >
                      Tải lại
                    </button>
                  </div>
                  {debugInfo && (
                    <div className="text-left mt-3 text-xs text-[#a1a1a8] bg-[#f8fafc] p-2 rounded">
                      <div>HTTP: {debugInfo.status ?? "-"}</div>
                      <div>Items: {debugInfo.length ?? "-"}</div>
                      <details className="mt-1 text-[11px]">
                        <summary>Raw payload</summary>
                        <pre className="whitespace-pre-wrap">{JSON.stringify(debugInfo.raw, null, 2)}</pre>
                      </details>
                    </div>
                  )}
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {tx.source && (
                                <span className="text-[10px] px-2 py-0.5 bg-[#eef2ff] text-[#4b5b9a] rounded-full font-semibold">
                                  {tx.source}
                                </span>
                              )}
                            </div>
                            ...
                            {tx.note && (
                              <p className="text-[11px] text-[#6b6d75] mt-1 truncate">
                                {tx.note}
                              </p>
                            )}
                          </div>
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <p className="text-[9px] text-[#767681] font-bold uppercase mt-0.5 flex items-center gap-2">
                            {tx.category}
                            {tx.isSettled && (
