...
function VerifyContent() {
  ...
  // =========================================
  // STATE: Quản lý thông báo Popup (UI Bản 2)
  // =========================================
  ...
  // =========================================
  // HANDLER: Xác nhận OTP (Logic API Bản 1)
  // =========================================
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification(
          "error",
          data.detail || "Mã xác thực không đúng hoặc đã hết hạn."
        );
      }
    } catch (error) {
  ...
  // =========================================
  // HANDLER: Gửi lại mã (Logic API Bản 1)
  // =========================================
    ...
    if (countdown === 0) {
      try {
        ...
        if (response.ok) {
          showNotification(
            "success",
            `Hệ thống đã gửi lại mã mới đến email: ${email}`
          );
          setCountdown(60);
        } else {
          showNotification(
  ...
  return (
    <>
      {/* =========================================
          THÔNG BÁO - Nổi giữa màn hình (UI Bản 2)
          ========================================= */}
      {notification.show && (
...
export default function VerifyPage() {
  ...
  return (
    // Thay min-h-screen bằng min-h-[100dvh] để trị lỗi bàn phím
    <div className="bg-[#f9f9fe] font-body text-[#1a1c1f] min-h-[100dvh] flex flex-col overflow-x-hidden relative">
      ...
      {/* Đổi justify-center thành phần lớn flex-col với overflow-y-auto 
